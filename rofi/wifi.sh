...
# Use terse output with ':' separator to handle SSIDs with spaces safely
LIST_RAW=$(nmcli -t -f SSID,SECURITY dev wifi list | sed '/^--:/d')
# Convert ":" to 2+ spaces for rofi display (still readable)
LIST=$(echo "$LIST_RAW" | sed 's/:/  /')
...
#echo "$CHENTRY"
# SSID is everything before the "  SECURITY" column we created
CHSSID="$(echo "$CHENTRY" | sed -E 's/[[:space:]]{2,}.*$//' )"
CHSSID="$(echo "$CHSSID" | sed 's/^[[:space:]]*//; s/[[:space:]]*$//')"
#echo "$CHSSID"
