...
require("conform").setup({
		...
		oxfmt = {
			args = function(_self, ctx)
				...
				local git_root   = vim.fn.systemlist(
					"git -C " .. vim.fn.shellescape(search_dir) .. " rev-parse --show-toplevel"
				)[1]
				...
				if not project_config then
					for _, candidate in ipairs({ vim.fn.expand("~/.oxfmtrc.jsonc"), vim.fn.expand("~/.oxfmtrc.json") }) do
						if vim.fn.filereadable(candidate) == 1 then
							project_config = candidate
							break
						end
				...
				if project_config then vim.list_extend(args, { "--config", project_config }) end
				return args
			end,
		},
		prettier = {
			args = function(_self, ctx)
				...
				for _, name in ipairs(config_files) do
					local found = vim.fn.findfile(name, (ctx.dirname or vim.fn.getcwd()) .. ";")
					if found and found ~= "" then project_config = found; break end
				end
	...
	format_on_save = function(bufnr)
		if vim.tbl_contains({ "sql" }, vim.bo[bufnr].filetype) then return end
...
vim.api.nvim_create_user_command("FormatDisable", function(opts)
	if opts.bang then vim.b.disable_autoformat = true else vim.g.disable_autoformat = true end
...
vim.keymap.set("n", "<leader>uf", function()
	...
	vim.cmd(auto_format and "FormatEnable" or "FormatDisable")
